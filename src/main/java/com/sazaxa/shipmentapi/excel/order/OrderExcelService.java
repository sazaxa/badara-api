    ...
    public ExcelSuccessResponseDto create(MultipartFile file) throws IOException {
        ...
        for (Row row : sheet){
            ...
            Recipient recipient = Recipient.builder()
                    ...
                    .address1(row.getCell(ADDRESS.ordinal()).getStringCellValue())
                    .zipcode(row.getCell(ZIPCODE.ordinal()).getStringCellValue())
                    .city(row.getCell(CITY.ordinal()).getStringCellValue())
                    .state(row.getCell(STATE.ordinal()).getStringCellValue())
                    .memo(row.getCell(RECIPIENT_MEMO.ordinal()).getStringCellValue())
            ...
            Box box = Box.builder()
                    ...
                    .userMemo(row.getCell(BOX_MEMO.ordinal()).getStringCellValue())
                    .order(order)
                    .build();
